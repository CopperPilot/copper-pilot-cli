...
def harden_state_dir(path: Path) -> None:
    ...
    for thread in uncached:
        ...
        if include_initial_prompt and "initial_prompt" not in thread:
            if thread_id in prompt_results:
                ...
                prompt = batch_results.get(thread_id, _CheckpointSummary(None, None)).initial_prompt
    ...
    for start in range(0, len(thread_ids), _SQLITE_MAX_VARIABLE_NUMBER):
        ...
        for row in rows:
            ...
            if not type_str or not checkpoint_blob:
                results[tid] = _CheckpointSummary(message_count=None, initial_prompt=None)
                continue
            try:
...
def _reduce_message_write_rows(
    ...
    for tid, type_str, value_blob in rows:
        ...
        try:
            ...
            logger.warning(
                "Failed to replay messages write for thread %s; message count may be inaccurate",
                tid,
                exc_info=True,
            )
            continue
...
def _summarize_checkpoint(data: object) -> _CheckpointSummary:
    ...
    return _CheckpointSummary(
        message_count=(_visible_message_count(messages) if messages is not None else None),
...
def get_thread_limit() -> int:
    ...
    """
    raw = os.environ.get("COPPER_PILOT_RECENT_THREADS")
    if raw is None:
